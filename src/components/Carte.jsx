...
import { IconArrowLeft, IconMapOff, IconCar, IconChevronDown, IconHelpCircle } from '@tabler/icons-react';
...
export function Carte({ voyage, setActiveTab, integree = false }) {
  ...
  // Toutes les étapes en option, géolocalisées ou non — affichées à part
  // dans le petit onglet "Options", jamais mêlées aux trajets confirmés.
  const optionsToutes = activites.filter((a) => a.option);

  // Seules les activités confirmées (pas "option") avec des coordonnées GPS
  // participent au tracé et au calcul des trajets. Une option, même
  // géolocalisée, reste un plan B tant qu'elle n'est pas confirmée : elle
  // ne doit pas fausser l'itinéraire réel.
  const pointsConfirmes = activites.filter((a) => !a.option && typeof a.lat === 'number' && typeof a.lon === 'number');
  // Les options géolocalisées sont quand même affichées sur la carte (sans
  // être reliées par une route), pour donner une idée de leur emplacement.
  const optionsGeolocalisees = optionsToutes.filter((a) => typeof a.lat === 'number' && typeof a.lon === 'number');
  ...
  const joursTries = [...new Set(pointsConfirmes.map((p) => p.date))].sort();
  const couleurJour = (date) => PALETTE_JOURS[joursTries.indexOf(date) % PALETTE_JOURS.length];

  // Distance et temps de route entre chaque étape confirmée consécutive,
  // via OSRM (service public gratuit, sans clé, basé sur OpenStreetMap).
  // Chaque segment est demandé séparément (plutôt qu'un seul gros trajet
  // groupé) pour récupérer le tracé réel de CHAQUE portion individuellement
  // — c'est ce qui permet de colorer chaque bout de route selon le jour.
  const [trajets, setTrajets] = useState([]);
  const [chargementTrajets, setChargementTrajets] = useState(false);
  const [erreurTrajets, setErreurTrajets] = useState(false);

  useEffect(() => {
    if (pointsConfirmes.length < 2) { setTrajets([]); setErreurTrajets(false); return; }

    const chercherTrajets = async () => {
      setChargementTrajets(true);
      setErreurTrajets(false);
      try {
        const requetes = pointsConfirmes.slice(0, -1).map(async (p, i) => {
          const a = pointsConfirmes[i + 1];
    ...
    chercherTrajets();
  }, [pointsConfirmes.map((p) => `${p.lat},${p.lon}`).join('|')]);

  // Total cumulé de tous les trajets confirmés — utile pour voir d'un coup
  // d'œil le volume de route sur l'ensemble du voyage.
  ...
  useEffect(() => {
    if (!conteneurRef.current || (pointsConfirmes.length === 0 && optionsGeolocalisees.length === 0)) return;
    ...
    const latlngsConfirmes = pointsConfirmes.map((p) => [p.lat, p.lon]);
    const latlngsTous = [...latlngsConfirmes, ...optionsGeolocalisees.map((p) => [p.lat, p.lon])];
    ...
    if (trajets.length > 0) {
    ...
    } else if (latlngsConfirmes.length > 1) {
      ...
      L.polyline(latlngsConfirmes, { color: '#B8863C', weight: 3, opacity: 0.5, dashArray: '6 8' }).addTo(carte);
    }

    // Étapes confirmées : marqueur plein, numéroté dans l'ordre du voyage
    pointsConfirmes.forEach((p, i) => {
    ...
    // Options géolocalisées : marqueur pointillé avec un "?", pas de numéro
    // d'étape puisqu'elles ne font pas (encore) partie de l'itinéraire fixé.
    optionsGeolocalisees.forEach((p) => {
      const couleur = COULEURS_CATEGORIE[p.categorie] || '#8A7B68';
      const emoji = EMOJI_CATEGORIE[p.categorie] || '📍';
      const icone = L.divIcon({
        html: `<div style="background:${couleur};color:#FFF;width:26px;height:26px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:700;font-family:sans-serif;border:2px dashed white;opacity:0.75;box-shadow:0 2px 6px rgba(0,0,0,0.3);">?</div>`,
        className: '',
        iconSize: [26, 26],
        iconAnchor: [13, 13]
      });
      L.marker([p.lat, p.lon], { icon: icone })
        .addTo(carte)
        .bindPopup(
          `<strong>${emoji} ${p.titre} (option)</strong><br/>${new Date(p.date).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' })} · ${p.heure || ''}`
        );
    });

    if (latlngsTous.length === 1) {
      carte.setView(latlngsTous[0], 14);
    } else if (latlngsTous.length > 1) {
      carte.fitBounds(latlngsTous, { padding: [40, 40] });
    }
  }, [pointsConfirmes, optionsGeolocalisees, trajets]);
  ...
  const aucunPointDuTout = pointsConfirmes.length === 0 && optionsGeolocalisees.length === 0;
    ...
    <div style={{ padding: integree ? '0' : '15px', fontFamily: 'inherit' }}>
      ...
      <div
        ...
        style={{
          ...
          display: (!chargement && aucunPointDuTout) ? 'none' : 'block'
        }}
      ></div>
      ...
      {trajets.length > 0 && (
        <div style={{ marginTop: '18px' }}>
          ...
          {/* Total cumulé de route sur l'ensemble du voyage (étapes confirmées uniquement) */}
      ...
      {/* Petit onglet dépliable listant les options : jamais mêlées aux
          trajets confirmés, qu'elles aient un lieu renseigné ou non. */}
      {optionsToutes.length > 0 && (
        <div style={{ marginTop: '18px' }}>
          <button
            type="button"
            ...
            style={{
              ...
              backgroundColor: '#F7F1E8', border: '1px dashed #C9BBA0', borderRadius: '14px',
              cursor: 'pointer', fontFamily: 'inherit'
            }}
          >
            <IconHelpCircle size={16} color="#8A7B68" />
            <span style={{ flex: 1, textAlign: 'left', fontSize: '13px', fontWeight: '800', color: '#8A7B68' }}>
              Options ({optionsToutes.length})
            </span>
            <IconChevronDown size={16} color="#B5A793" style={{ transform: optionsOuvertes ? 'rotate(180deg)' : 'rotate(0deg)', transition: 'transform 0.2s ease' }} />
          </button>

          {optionsOuvertes && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginTop: '10px' }}>
              {optionsToutes.map((o) => {
                const emoji = EMOJI_CATEGORIE[o.categorie] || '📍';
                const geolocalisee = typeof o.lat === 'number' && typeof o.lon === 'number';
                return (
                  <div key={o.id} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', backgroundColor: '#FFFFFF', borderRadius: '12px', border: '1.5px dashed #E8DFCF' }}>
                    <span style={{ fontSize: '16px', flexShrink: 0 }}>{emoji}</span>
                    <span style={{ flex: 1, minWidth: 0 }}>
                      <span style={{ display: 'block', fontSize: '12.5px', color: '#2B2420', fontWeight: '600', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                        {o.titre}
                      </span>
                      <span style={{ display: 'block', fontSize: '11px', color: '#B5A793', marginTop: '1px' }}>
                        {new Date(o.date).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' })} · {o.heure || '--:--'}
                        {!geolocalisee && ' · lieu non renseigné'}
                      </span>
                    </span>
                  </div>
                );
