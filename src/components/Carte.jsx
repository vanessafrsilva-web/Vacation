...
export function Carte({ voyage, setActiveTab, integree = false, masquerOptions = false }) {
  ...
  // Quand "Masquer les options" est actif dans le Planning, on retire ces
  // étapes de la carte ET du calcul des trajets — les segments sont alors
  // recalculés directement entre les étapes confirmées adjacentes.
  const pointsAffiches = masquerOptions ? points.filter((p) => !p.option) : points;
  ...
  const joursTries = [...new Set(pointsAffiches.map((p) => p.date))].sort();
  ...
  useEffect(() => {
    if (pointsAffiches.length < 2) { setTrajets([]); setErreurTrajets(false); return; }

    const chercherTrajets = async () => {
      setChargementTrajets(true);
      setErreurTrajets(false);
      try {
        const requetes = pointsAffiches.slice(0, -1).map(async (p, i) => {
          const a = pointsAffiches[i + 1];
    ...
    chercherTrajets();
  }, [pointsAffiches.map((p) => `${p.lat},${p.lon}`).join('|')]);
  ...
  useEffect(() => {
    if (!conteneurRef.current || pointsAffiches.length === 0) return;
    ...
    const latlngs = pointsAffiches.map((p) => [p.lat, p.lon]);
    ...
    if (trajets.length > 0) {
      ...
      // Un segment qui touche une étape "option" (encore visible, pas
      // masquée) est tracé en pointillés plus fins pour signaler son
      // caractère incertain, cohérent avec le style du Planning.
      trajets.forEach((t) => {
        const couleur = couleurJour(t.de.date);
        const estIncertain = t.de.option || t.a.option;
        if (t.geometrie && t.geometrie.length > 1) {
          L.polyline(t.geometrie, {
            color: couleur,
            weight: estIncertain ? 3 : 4,
            opacity: estIncertain ? 0.5 : 0.85,
            dashArray: estIncertain ? '6 8' : null
          }).addTo(carte);
        } else {
    ...
    pointsAffiches.forEach((p, i) => {
      const couleur = COULEURS_CATEGORIE[p.categorie] || '#8A7B68';
      const emoji = EMOJI_CATEGORIE[p.categorie] || '📍';
      // Une étape "option" est affichée avec une bordure en pointillés et
      // une opacité réduite, pour rester cohérente avec le Planning.
      const styleMarqueur = p.option
        ? `background:${couleur};color:#FFF;width:30px;height:30px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:700;font-family:sans-serif;border:2px dashed white;opacity:0.75;box-shadow:0 2px 6px rgba(0,0,0,0.3);`
        : `background:${couleur};color:#FFF;width:30px;height:30px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:700;font-family:sans-serif;border:2px solid white;box-shadow:0 2px 6px rgba(0,0,0,0.3);`;
      const icone = L.divIcon({
        html: `<div style="${styleMarqueur}">${i + 1}</div>`,
        className: '',
        iconSize: [30, 30],
        iconAnchor: [15, 15]
      });
      L.marker([p.lat, p.lon], { icon: icone })
        .addTo(carte)
        .bindPopup(
          `<strong>${emoji} ${p.titre}${p.option ? ' (option)' : ''}</strong><br/>${new Date(p.date).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' })} · ${p.heure || ''}`
        );
    });
  ...
  return (
    <div style={{ padding: integree ? '0' : '15px', fontFamily: 'inherit' }}>
      ...
      {!chargement && pointsAffiches.length === 0 && (
        <div style={{ padding: '40px 20px', textAlign: 'center', backgroundColor: '#FFFFFF', borderRadius: '20px', border: '1px dashed #E8DFCF' }}>
          ...
          <p style={{ color: '#8A7B68', fontSize: '13.5px', margin: 0, lineHeight: '1.6' }}>
            {points.length > 0 && masquerOptions
              ? "Uniquement des étapes en option sur ce voyage — masquées pour l'instant."
      ...
      <div
        ...
        style={{
          ...
          display: (!chargement && pointsAffiches.length === 0) ? 'none' : 'block'
        }}
      ></div>
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    border: estIncertain ? '1.5px dashed #C9BBA0' : '1px solid #E8DFCF',
                    opacity: estIncertain ? 0.75 : 1
                  }}
                >
                  ...
                  <span style={{ flex: 1, fontSize: '12.5px', color: '#2B2420', fontWeight: '600', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                    {i + 1}. {t.de.titre}{t.de.option ? ' (option)' : ''} → {i + 2}. {t.a.titre}{t.a.option ? ' (option)' : ''}
                  </span>
